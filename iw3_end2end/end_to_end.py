...
class DPT_DINOv2(nn.Module):
    def __init__(self, encoder='vitl',
        ...
        if localhub:
            dinov2_path = os.path.join(os.path.dirname(__file__), "pretrained_models", "hub",
                                       "nagadomi_Depth-Anything_iw3_main", "torchhub", "facebookresearch_dinov2_main")
...
def load_depth_model(device):
    ...
    try:
        # 创建模型实例
        model = DPT_DINOv2(encoder="v2_vits", localhub=True)
        ...
        # 下载并加载权重
        checkpoint_path = os.path.join(torch.hub.get_dir(), "checkpoints", file_name)
